...
// delete function code below
const deleteFunc = (i) => {
  let filteredData = orderDetailLS.filter((el, ind) => {
    return ind !== i;
  });
  localStorage.setItem("orderDetailLS", JSON.stringify(filteredData));
};
// *****************************

// pagination code below
let pageCount = 1;
const paginationFunc = (val) => {
  if (val === "inc") {
    pageCount++;
  } else if (val === "dec") {
    pageCount--;
  }
  document.getElementById("currentBtn").innerText = pageCount;
  let paginatedData = orderDetailLS.filter((el, i) => {
...
document
  ...
  .addEventListener("click", () => paginationFunc("inc"));
document
  .getElementById("prevBtn")
  .addEventListener("click", () => paginationFunc("dec"));
